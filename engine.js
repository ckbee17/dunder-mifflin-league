...
const META=[
 {id:'jim',name:'Jim Halpert',role:'Sales',strat:'The closer — bets big on the sure thing',quote:"You don't force the sale. You close the one that's already yours."},
...
function loadState(){
 try{ const s=JSON.parse(globalThis.localStorage.getItem(KEY)); if(s&&s.agents){ IDS.forEach(k=>{ if(!s.records[k]) s.records[k]={daily:[],days_led:0,best_day:0,worst_day:0,brier_sum:0,brier_n:0,streak:0,peak:START}; if(s.agents[k]&&!s.agents[k].log) s.agents[k].log=[]; if(s.agents[k]&&s.agents[k].day_spent==null) s.agents[k].day_spent=0; if(s.agents[k]&&!s.agents[k].tier) s.agents[k].tier='main'; if(s.agents[k]&&s.agents[k].flip==null) s.agents[k].flip=false; if(s.agents[k]&&s.agents[k].size_mult==null) s.agents[k].size_mult=(ROSTER[k]&&ROSTER[k].size_mult)||1; if(s.agents[k]&&s.agents[k].win_tier==null) s.agents[k].win_tier=Math.floor((s.agents[k].wins||0)/500); }); return s; } }catch(e){}
 return freshState();
}
...
function buy(a,s,side,budget){
 budget=Math.min(budget,a.cash*0.95); if(budget<MIN_BET) return 0;   // no dollar-dust bets — skip if can't place a real one
...
async function runRound(){
 ...
 IDS.forEach(id=>{ const a=st.agents[id]; a.today_trades=0;
  ...
  picks.forEach(pr=>{ const s=pr[0], side=pr[1], key=s.id+'|'+side;
    if(key in a.positions) return;                          // already holding this market — one bet per market
    if(Object.keys(a.positions).length>=MAX_OPEN) return;   // hard cap: 10 open positions at a time
    if(buy(a,s,side,BET_SIZE*(a.size_mult||1))>0) a.today_trades++; }); });
 ...
 IDS.forEach(id=>{ const a=st.agents[id]; const eq=st.last_equity[id];
  if(a.tier!=='warehouse' && a.cash*0.95<MIN_BET && Object.keys(a.positions).length===0){ // can't afford the minimum bet → sent down, strategy flips
...
function rows(st){
 return IDS.map(id=>{ const a=st.agents[id], r=st.records[id], m=METABY[id];
  ...
  return {id,name:m.name,role:m.role,strat:m.strat,quote:m.quote,
   ...
   betSize:Math.round(BET_SIZE*(a.size_mult||1)),sizeMult:(a.size_mult||1)}; });
}
 ...
 el.innerHTML=R.map((t,i)=>{ const cl=i<3?'p'+(i+1):'', settled=t.wins+t.losses;
  return '<div class="card '+cl+(t.id===selected?' sel':'')+'" data-id="'+t.id+'">'+
   ...
   '<div class="cardstats"><span>Record <b>'+t.wins+'-'+t.losses+'</b></span>'+
     ...
     '<span>Bet <b'+(t.sizeMult>1?' class="g"':'')+'>$'+t.betSize+(t.sizeMult>1?' ·'+t.sizeMult+'×':'')+'</b></span>'+
...
var POS={ jim:[22,46], pam:[18,54], dwight:[26,55], andy:[35,45], phyllis:[33,56], stanley:[40,56],
 ...
 meredith:[22,84], creed:[33.6,82], darryl:[41,82], ryan:[57,66], gabe:[89,56], toby:[93,66], kelly:[92,83], infinity:[92.8,40] };
...
function buildOffice(){
 ...
 document.getElementById('spriteLayer').innerHTML=IDS.map(function(id){ return '<div class="sprite" id="spr-'+id+'" data-id="'+id+'">'+spriteSVG(SPR[id])+'<div class="tip"></div></div>'; }).join('')+'<div class="sprite inf" id="spr-infinity" data-id="infinity" style="left:'+POS.infinity[0]+'%;top:'+POS.infinity[1]+'%">'+infinitySVG()+'<div class="tip"></div></div>';
 var wb=document.getElementById('whiteboard'); wb.style.left='52%'; wb.style.top='11%'; wb.style.width='22%';
 OFFICE.start();
}

/* ---------- Dunder Mifflin Infinity: the live computer trader ----------
   A display-only competitor. NOT in IDS, so the paper engine never trades or
   moves it. Its numbers come from live.json, written by the user's own
   trading bot after each run. The dashboard only READS and displays. */
var INF={id:'infinity',name:'Dunder Mifflin Infinity',role:'E-Commerce · The Website',
 quote:"I don't take Pretzel Day. I don't take lunch. I just execute.",
 strat:'The machine — real money, trading the winning-staff playbook'};
var LIVE=null;
var INFLOGO='data:image/jpeg;base64,';
function infinitySVG(){var s='';
 s+=px(10,19,2,4,'#9aa4b0')+px(7,23,8,2,'#9aa4b0')+px(6,25,10,1,'#79838f');
 s+=px(3,4,16,16,'#79838f')+px(4,5,14,14,'#c6ced6');
 s+=px(6,7,10,9,'#0e1c30');
 s+=px(8,9,2,2,'#8fe3ff')+px(12,9,2,2,'#8fe3ff');
 s+=px(6,14,1,1,'#35c46a')+px(8,13,1,1,'#35c46a')+px(10,12,1,1,'#35c46a')+px(12,11,1,1,'#35c46a')+px(13,10,2,2,'#35c46a');
 s+=px(5,17,12,2,'#2f6fd6');
 return '<svg viewBox="0 0 22 31" xmlns="http://www.w3.org/2000/svg">'+s+'</svg>';}
function fetchLive(){
...
function buildInfinity(){
 if(typeof document==='undefined'||document.getElementById('infinity')) return;
 var tabs=document.querySelector('.tabs');
 if(tabs){ var b=document.createElement('button'); b.className='tab'; b.setAttribute('data-tab','infinity'); b.textContent='DM Infinity'; tabs.appendChild(b); }
 var sec=document.createElement('section'); sec.id='infinity'; sec.hidden=true;
 sec.innerHTML='<div class="infhead"><img class="inflogo" src="'+INFLOGO+'" alt="Dunder Mifflin Infinity">'+
   '<div class="inftt"><div class="inftitle">DUNDER MIFFLIN INFINITY <span class="inflive">● LIVE</span></div>'+
   '<p>The website joined the sales floor. Real money, trading the winning-staff playbook — the staff versus the computer.</p></div></div>'+
   '<div id="infbody"></div>';
 var anchor=document.getElementById('warehouse')||document.getElementById('office')||document.getElementById('conf');
 anchor.parentNode.insertBefore(sec, anchor.nextSibling);
 if(!document.getElementById('infcss')){ var st=document.createElement('style'); st.id='infcss';
  st.textContent='.infhead{display:flex;gap:16px;align-items:center;margin:2px 0 16px}'+
   '.inflogo{height:52px;border-radius:4px}'+
   '.inftitle{font-family:var(--disp,inherit);font-size:21px;letter-spacing:.6px;color:#eaf1fb;display:flex;align-items:center;gap:10px}'+
   '.inflive{font-size:11px;font-weight:700;color:#7CFFB0;letter-spacing:.5px;animation:infpulse 2s infinite}'+
   '@keyframes infpulse{0%,100%{opacity:1}50%{opacity:.35}}'+
   '.inftt p{margin:3px 0 0;font-size:13px;color:#aebccb;max-width:640px}'+
   '.infgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:14px}'+
   '.inftile{background:var(--card,#fff);border:1px solid var(--paperline,#dcd);border-radius:8px;padding:14px 16px}'+
   '.inftile .k{font-size:11.5px;text-transform:uppercase;letter-spacing:.5px;color:var(--faint,#889)}'+
   '.inftile .v{font-size:26px;font-weight:800;margin-top:4px;font-family:var(--disp,inherit)}'+
   '.infnote{font-size:13px;color:#aebccb;background:var(--card,#fff);border:1px dashed var(--paperline,#dcd);border-radius:8px;padding:12px 14px}'+
   '.infvs{margin-top:12px;font-size:13.5px;color:#c8d4e0}'+
   '.infpaused{background:#3a1e12;border:1px solid #c0392b;color:#ffcf5c;border-radius:8px;padding:11px 14px;margin-bottom:12px;font-weight:700;font-size:14px;text-align:center;letter-spacing:.3px}'+
   '.infquote{margin-top:10px;font-style:italic;color:#93a3b4;font-size:13px}'+
   '.sprite.inf .tip{white-space:nowrap}';
  document.head.appendChild(st); }
}
function renderInfinity(L,Rmain){
 if(typeof document==='undefined') return;
 var el=document.getElementById('infbody'); if(!el) return;
 L=L||LIVE||{};
 var dep=(L.deposited!=null?L.deposited:550), bal=(L.balance!=null?L.balance:dep);
 var pnl=(L.pnl!=null?L.pnl:(bal-dep));
 var wins=L.wins||0, losses=L.losses||0, settled=wins+losses;
 var bets=(L.bets!=null?L.bets:settled);
 var wr=settled?Math.round(100*wins/settled):null;
 var openN=(L.positions&&L.positions.length!=null)?L.positions.length:(L.open!=null?L.open:0);
 var fresh; if(!L.updated){ fresh='standing by — no report yet'; }
 else { var m=Math.round((Date.now()-new Date(L.updated).getTime())/60000); fresh=(m<1?'updated just now':'updated '+m+'m ago'); }
 var pnlc=pnl>=0?'var(--gain,#1a9d5a)':'var(--loss,#c0392b)';
 var paused=L.paused_manual||L.paused;
 var banner=paused?('<div class="infpaused">⏸ PAUSED'+(L.paused_manual?' — manually stopped':'')+' · not placing new trades</div>'):'';
 var h=banner+'<div class="infgrid">'+
  '<div class="inftile"><div class="k">Balance</div><div class="v">'+money(bal)+'</div></div>'+
  '<div class="inftile"><div class="k">Net P&amp;L</div><div class="v" style="color:'+pnlc+'">'+signed(pnl)+'</div></div>'+
  '<div class="inftile"><div class="k">Win rate</div><div class="v">'+(wr!=null?wr+'%':'—')+'</div></div>'+
  '<div class="inftile"><div class="k">Bets settled</div><div class="v">'+settled+'</div></div>'+
  '<div class="inftile"><div class="k">Open now</div><div class="v">'+openN+'</div></div>'+
  '</div>';
 if(!L.updated){
  h+='<div class="infnote">Seeded at '+money(dep)+'. Waiting for the first report from your trading bot — the moment it writes <b>live.json</b>, this board lights up automatically. No manual updates.</div>';
 } else {
  // ladder: next size-up at the next multiple of 200 settled bets, if win rate holds >=95%
  var nextMs=(Math.floor(settled/200)+1)*200;
  var gate=(wr!=null&&wr>=95)?('on track — '+(nextMs-settled)+' bets to the next size-up'):('win rate below 95% — size-ups paused until it recovers');
  h+='<div class="infnote">Milestone ladder: every 200 settled bets at 95%+, the bot steps its size up. <b>'+gate+'.</b> '+fresh+'.</div>';
 }
 // vs the staff: where Infinity's return would rank on the paper board
 if(Rmain&&Rmain.length){
  var infPct=dep?pnl/dep:0;
  var better=Rmain.filter(function(t){return (t.pnl/1000)>infPct;}).length;
  var rank=better+1;
  h+='<div class="infvs">If Infinity were on the paper board, its <b>'+(infPct>=0?'+':'')+(infPct*100).toFixed(1)+'%</b> return would rank <b>#'+rank+'</b> against the '+Rmain.length+' staff.</div>';
 }
 h+='<div class="infquote">“'+INF.quote+'”</div>';
 el.innerHTML=h;
 // update the floor sprite's hover tip
 var sp=document.getElementById('spr-infinity'); if(sp){ var tip=sp.querySelector('.tip'); if(tip){ tip.innerHTML='<b>DM Infinity</b> · '+money(bal)+' ('+signed(pnl)+')'+(wr!=null?' · '+wr+'% W':''); } }
}
...
function buildControls(){
 ...
 if(tabsEl) tabsEl.addEventListener('click',function(e){ var btn=e.target.closest&&e.target.closest('.tab'); if(!btn) return;
   ...
   var t=btn.dataset.tab; ['bullpen','conf','office','warehouse','infinity'].forEach(function(id){ var el=document.getElementById(id); if(el) el.hidden=(id!==t); });
...
function fetchState(){
 ...
 fetch('state.json?t='+Date.now(),{cache:'no-store'}).then(function(r){return r.json();}).then(function(st){
   ...
   render(loadState(),{},LAST_SNAPS);
   try{ renderInfinity(LIVE, rows(loadState()).sort(function(a,b){return b.balance-a.balance;})); }catch(e){}
...
function initViewer(){
 ...
 buildOffice(); buildWarehouse(); buildInfinity(); buildOpenPanel(); buildModal(); wireInteractions();
